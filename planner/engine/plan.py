...
def _failed_plan_result(exit_code: int, errors: list[str]) -> PlanResult:
    return PlanResult(
        exit_code=exit_code,
...
def _run_successful_plan_search(errors: list[str], runtime: _PlanRuntime) -> Optimal | PlanResult:
    ...
    if isinstance(result, Indeterminate):
        diagnostics = list(result.diagnostics) or ["canonical optimization is indeterminate"]
        errors.extend(f"plan: {diagnostic}" for diagnostic in diagnostics)
        return _failed_plan_result(1, errors)
...
def _write_successful_plan(
    ...
    try:
        ...
        message = "plan: canonical publication interrupted"
        print(message, file=sys.stderr)
        errors.append(message)
        return _failed_plan_result(1, errors)
    except Exception as error:  # noqa: BLE001
