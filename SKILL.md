...
4. For substance relations, write both sides: `balance` on both related cards, `supports` on the supporter/cofactor card, and `supported_by` on the main/target substance card. Do not invent relation facts without a source or explicit user decision.
