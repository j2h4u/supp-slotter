...
- For recommendation/proposal work, pick reusable axes before products: `is:`, `effect:`, `risk:`, `pathway:`, relations, or dashboard projections.
...
11. Choose relation endpoint fields by how broad each side is:
   ...
   - `source_trait` / `target_trait`: every substance carrying a registered `namespace:slug`, only when the relation is genuinely category-level and future members should inherit it. `planner review` shows the concrete active source/target matches for these endpoints. Slot-blocking rules belong in ontology scheduling constraints, not `data/relations.yaml`.
...
- Source-completion work: run `uv run python -m planner audit --full` only when product URLs, label notes, forms, or component amounts matter for the current task.
...
- `planner audit` — diagnostics (valid knowledge-only substance cards, products outside stacks, unused traits, potential duplicate cards, empty clusters) and optional `--full` product source/amount drilldown for tasks where those facts matter.
...
- `full.active_product_source` — active product cards missing source URLs, product notes, brand, or component amounts. Treat this as source-completion guidance, not automatic debt. Unknown amounts are acceptable when the current decision does not depend on them. Add a `data_quality` concern only when source conflict or missing label data blocks a concrete review or product decision.
...
Use `planner review` first: its `Review brief` is the review intake surface. Use `schedule.yaml` for slot placement. Use `audit --full` only when product/source/amount drilldown matters for the review.
