...
|---|---|---|---|
| Long-term zinc and copper status should be reviewed together. | `balance` on zinc and copper cards. | Fits now; `plan`/`doctor` can warn when one side is active and the other is absent. | Encoded active |
| Zinc and copper can compete when co-administered. | `competes` on zinc and copper cards. | Fits now; planner avoids co-slotting. | Encoded active |
| NAC is supported by selenium and molybdenum in the Doctor's Best formula context. | `supported_by` on NAC; `supports` on selenium and molybdenum forms. | Fits now; `plan`/`doctor` can warn when NAC is active without its declared supporters. | Encoded inactive |
| Magnesium supports calcium homeostasis. | Currently encoded on the active Magnesium (glycinate) card; this represents magnesium-level behavior, not a glycinate-specific claim. | Fits now. | Encoded active |
| Vitamin E can support vitamin A bioavailability/absorption. | `supports` from vitamin E tocopherol to vitamin A. | Fits now. | Encoded inactive |
| Vitamin E helps protect omega-3 PUFA from lipid peroxidation. | `supports` from vitamin E tocopherol to EPA. | Fits now. | Encoded active |
| High-dose vitamin E can antagonize vitamin K-dependent clotting factors. | `antagonizes` on vitamin E; `antagonized_by` on K1 and K2. | Fits now as review relation. | Encoded inactive |
...
| Calcium and vitamin D are synergistic; vitamin D improves calcium absorption. | `supports` from vitamin D to calcium and `supported_by` on calcium if this review warning becomes useful. | Fits now. | Candidate |
...
| L-Arginine, L-ornithine, and L-lysine may synergize around growth hormone. | `supports`/`supported_by` if reviewing each substance; `prefer_with` only if planner co-location is desired. | Fits with judgement. | Candidate |
...
| Vitamin C supports collagen synthesis through L-lysine/L-proline hydroxylation. | `supports` from vitamin C to L-lysine/L-proline collagen context, or a goal-cluster note. | Fits with judgement. | Candidate |
