...
| Vitamin E forms plus astaxanthin may synergize around keratinization / skin barrier; skin review also benefits from vitamin C, zinc, omega-3, and MSM context. | Encode as a `skin_support` dashboard cluster, not a `supports` relation. This is broader than barrier-only review, but narrower than all connective-tissue support. | `data/dashboards/skin_support.yaml` via `from_traits:`. |
