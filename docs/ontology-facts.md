...
|---|---|---|
| Vitamin E and vitamin K compete; separate by 2 hours / different meals. | Use `review_with` only — review-warning is sufficient. Do not promote to `competes` until a dose model exists; the operator does not use mega-doses, so slot separation would always be overkill. ✓ `review_with` now emits `review_with_substance_present` when both endpoints are active. | `data/relations.yaml` `review_with`: Vit E → Vit K1, Vit E → Vit K2. |
| Vitamins A and E may reduce vitamin K cellular uptake/function; separate 2-3 hours. | Same policy as Vit E / Vit K above — `review_with` only. ✓ `review_with` now emits `review_with_substance_present` when both endpoints are active. | `data/relations.yaml` `review_with`: Vit A → Vit K1, Vit A → Vit K2 (Vit E → Vit K already encoded). |
...
| Adaptogen × SSRI/SNRI/MAOI adverse-event interaction (e.g., Ashwagandha + escitalopram/sertraline/paroxetine reports per PMC10565488). | Defer concrete `review_with` encoding until any 5-HT-active medication enters the stack; the conditional trigger lives in the dashboard risk text and substance `concerns` (kind: safety). Cluster membership is projected from `risk: serotonergic_med_interaction` on relevant substance cards. | `data/dashboards/serotonergic_load.yaml` + Ashwagandha substance `concerns[kind=safety]`. |
...
- `review_with` is the right level for dose-dependent functional opposition:
  the planner emits a `review_with_substance_present` schedule warning when
...
| Functional opposition versus slot separation, for example vitamin A/E/K or receptor competition. | Use `review_with` for opposition where both-endpoints-active warrants a schedule warning (`review_with_substance_present`); use `competes` only when co-slotting should be avoided at typical doses. Slot placement for review_with remains unchanged — the warning is informational. | Keep both relation types; do not add another type until a concrete fact cannot choose between them. |
...
- Use `review_with` for pairings that should produce a schedule warning when
  both endpoints are simultaneously active: functional opposition, additive
  pharmacology, nutrient-status effects, medication interactions, or practical
  separation advice that is dose-dependent and cannot be computed by the planner.
  The planner emits a `review_with_substance_present` warning whenever both
  endpoints are active; slot placement is still unchanged.
