...
4. Neutral means an assessed in-model candidate has no directed scheduling outcome. Unresolved_without_direction means incomplete evidence makes no directed hypothesis. Outside_model has an explicit typed reason and no scheduling effect. None silently substitutes for coverage.
...
- [ ] Replace the one-outcome grooming receipt with a closed deterministic candidate/claim workflow that serves unresolved active role cards and claims in stable order and requires a typed disposition plus required fields before completion.
  ...
  - Evidence: CLI smoke shows the next unresolved card/claim; schema fixtures reject prose-only completion, missing pressure provenance, and untyped closure.
